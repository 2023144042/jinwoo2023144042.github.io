...
let yRotSpeed = 90.0;  // y-axis rotation speed (degree/sec)
...
function render() {
    ...
    const angle = glMatrix.toRadian(yRotSpeed * elapsedTime);
    const y=(5 + 5 * Math.sin(angle/2));
    const ex = Math.sin(angle) * 3.0;
    const ey = y;
    const ez = Math.cos(angle) * 3.0;

    eye = vec3.fromValues(ex, ey, ez);
